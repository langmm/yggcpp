...
from ._YggInterface import (
    ...
    YGG_MSG_MAX,  # noqa: F401
    YGG_MSG_EOF,  # noqa: F401
    YGG_MSG_BUF,  # noqa: F401
)
...
def InterfaceComm(name, flags=0, global_scope=False, **kwargs):
    r"""Short hand for initializing a default comm for use as an interface.

    Args:
        name (str): The name of the message queue.
        flags (int, optional): Bit flags controlling the communicator.
    ...
    flags |= COMM_FLAG.COMM_FLAG_INTERFACE
    out = Comm_t(name, flags=flags, **kwargs)
    if global_scope:
...
def YggInput(name, format_str=None, **kwargs):
    ...
    kwargs.update(direction=DIRECTION.RECV, recv_timeout=False)
...
def YggOutput(name, format_str=None, **kwargs):
    ...
    if format_str is not None:
        kwargs['format_str'] = format_str
    kwargs.update(direction=DIRECTION.SEND)
    return InterfaceComm(name, **kwargs)
...
def YggRpcServer(name, infmt=None, outfmt=None, **kwargs):
    ...
    kwargs.update(icomm_kwargs, commtype=COMM_TYPE.SERVER_COMM,
...
def YggRpcClient(name, outfmt=None, infmt=None, **kwargs):
    ...
    if outfmt is not None:
        ocomm_kwargs['format_str'] = outfmt
    kwargs.update(ocomm_kwargs, commtype=COMM_TYPE.CLIENT_COMM,
...
def YggTimesyncServer(name='timesync', **kwargs):
    ...
    """
    kwargs.update(commtype=COMM_TYPE.SERVER_COMM,
                  direction=DIRECTION.RECV)
    kwargs.setdefault('recv_timeout', False)
    return InterfaceComm(name, **kwargs)
...
def YggTimesync(name='timesync', **kwargs):
    ...
    """
    kwargs.update(commtype=COMM_TYPE.CLIENT_COMM,
                  direction=DIRECTION.SEND)
    kwargs.setdefault('recv_timeout', False)
    return InterfaceComm(name, **kwargs)
...
def YggAsciiTableInput(name, as_array=False, **kwargs):
    ...
    """
    return YggInput(name, as_array=as_array, **kwargs)
...
def YggAsciiTableOutput(name, fmt=None, as_array=False, **kwargs):
    ...
    return YggOutput(name, as_array=as_array, **kwargs)
