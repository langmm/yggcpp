...
int ObjDict::alloc(int i_nvert, int i_ntexc, int i_nnorm, int i_nparam,
                   int i_npoint, int i_nline, int i_nface, int i_ncurve, int i_ncurve2,
                   int i_nsurf, int do_color) {
    int i;
    // TODO  clear(); // Ensure that existing data is freed
    nvert = i_nvert;
    ntexc = i_ntexc;
    nnorm = i_nnorm;
    nparam = i_nparam;
    ...
    if (nvert > 0) {
        ...
        if (new_vert == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate vertices.");
            clear();
            return -1;
        }
        vertices = new_vert;
        for (i = 0; i < nvert; i++) {
            ...
            if (ivert == nullptr) {
                utils::ygglog_error("alloc_obj: Failed to allocate vertex %d.", i);
                clear();
                return -1;
            }
            vertices[i] = ivert;
        }
        utils::ygglog_debug("alloc_obj: Allocated %d vertices.", nvert);
    }
    // Allocate vertex colors
    if ((nvert > 0) && (do_color)) {
        int **new_vert_colors = (int **) malloc(nvert * sizeof(int *));
        if (new_vert_colors == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate vertex_colors.");
            clear();
            return -1;
        }
        ...
        for (i = 0; i < nvert; i++) {
            ...
            if (ivert == nullptr) {
                utils::ygglog_error("alloc_obj: Failed to allocate vertex color %d.", i);
                clear();
                return -1;
            }
            vertex_colors[i] = ivert;
        }
        utils::ygglog_debug("alloc_obj: Allocated %d vertex colors.", nvert);
    }
    // Allocate texcoords
    if (ntexc > 0) {
        float **new_texc = (float **) malloc(ntexc * sizeof(float *));
        if (new_texc == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate texcoords.");
            clear();
            return -1;
        }
        texcoords = new_texc;
        for (i = 0; i < ntexc; i++) {
            ...
            if (itexc == nullptr) {
                utils::ygglog_error("alloc_obj: Failed to allocate texcoord %d.", i);
                clear();
                return -1;
            }
            texcoords[i] = itexc;
        }
        utils::ygglog_debug("alloc_obj: Allocated %d texcoords.", ntexc);
    }
    // Allocate normals
    if (nnorm > 0) {
        float **new_norm = (float **) malloc(nnorm * sizeof(float *));
        if (new_norm == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate normals.");
            clear();
            return -1;
        }
        normals = new_norm;
        for (i = 0; i < nnorm; i++) {
            ...
            if (inorm == nullptr) {
                utils::ygglog_error("alloc_obj: Failed to allocate normal %d.", i);
                clear();
                return -1;
            }
            normals[i] = inorm;
        }
        utils::ygglog_debug("alloc_obj: Allocated %d normals.", nnorm);
    }
    // Allocate parameters
    if (nparam > 0) {
        float **new_param = (float **) malloc(nparam * sizeof(float *));
        if (new_param == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate params.");
            clear();
            return -1;
        }
        params = new_param;
        for (i = 0; i < nparam; i++) {
            ...
            if (iparam == nullptr) {
                utils::ygglog_error("alloc_obj: Failed to allocate param %d.", i);
                clear();
                return -1;
            }
            params[i] = iparam;
        }
        utils::ygglog_debug("alloc_obj: Allocated %d params.", nparam);
    }
    // Allocate points
    if (npoint > 0) {
        int **new_point = (int **) malloc(npoint * sizeof(int *));
        if (new_point == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate points.");
            clear();
            return -1;
        }
        points = new_point;
        ...
        if (new_nvert_point == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate nvert_in_point.");
            clear();
            return -1;
        }
    ...
    if (nline > 0) {
        ...
        if (new_line == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate lines.");
            clear();
            return -1;
        }
        lines = new_line;
        ...
        if (new_nvert_line == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate nvert_in_line.");
            clear();
            return -1;
        }
        ...
        if (new_line_texcoords == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate line_texcoords.");
            clear();
            return -1;
        }
    ...
    if (nface > 0) {
        ...
        if (new_face == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate faces.");
            clear();
            return -1;
        }
        faces = new_face;
        ...
        if (new_nvert_face == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate nvert_in_face.");
            clear();
            return -1;
        }
        ...
        if (new_face_texcoords == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate face_texcoords.");
            clear();
            return -1;
        }
        ...
        if (new_face_normals == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate face_normals.");
            clear();
            return -1;
        }
        ...
        utils::ygglog_debug("alloc_obj: Allocated %d faces.", nface);
    }
    // Allocate curves
    if (ncurve > 0) {
        int **new_curve = (int **) malloc(ncurve * sizeof(int *));
        if (new_curve == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate curves.");
            clear();
            return -1;
        }
        curves = new_curve;
        ...
        if (new_nvert_curve == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate nvert_in_curve.");
            clear();
            return -1;
        }
        ...
        if (new_curve_params == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate curve_params.");
            clear();
            return -1;
        }
        ...
        for (i = 0; i < ncurve; i++) {
            ...
            if (iparam == nullptr) {
                utils::ygglog_error("alloc_obj: Failed to allocate curve param %d.", i);
                clear();
                return -1;
            }
    ...
    if (ncurve2 > 0) {
        ...
        if (new_curve2 == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate curves2.");
            clear();
            return -1;
        }
        curves2 = new_curve2;
        ...
        if (new_nparam_curve2 == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate nparam_in_curve2.");
            clear();
            return -1;
        }
    ...
    if (nsurf > 0) {
        ...
        if (new_surface == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate surfaces.");
            clear();
            return -1;
        }
        ...
        if (new_nvert_surface == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate nvert_in_surface.");
            clear();
            return -1;
        }
        ...
        if (new_surface_params_u == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate surface_params_u.");
            clear();
            return -1;
        }
        ...
        for (i = 0; i < nsurf; i++) {
            float *iparam = (float *) malloc(2 * sizeof(float));
            if (iparam == nullptr) {
                utils::ygglog_error("alloc_obj: Failed to allocate surface param %d.", i);
                clear();
                return -1;
            }
        ...
        if (new_surface_params_v == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate surface_params_v.");
            clear();
            return -1;
        }
        ...
        if (new_surface_texcoords == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate surface_texcoords.");
            clear();
            return -1;
        }
        ...
        if (new_surface_normals == nullptr) {
            utils::ygglog_error("alloc_obj: Failed to allocate surface_normals.");
            clear();
            return -1;
        }
    ...
    // Return
    utils::ygglog_debug("alloc_obj: Allocated for\n"
                 "\t%d vertices,\n"
...
ObjDict::ObjDict(const ObjDict *src) : ObjDict() {
    ...
    alloc(src->nvert, src->ntexc, src->nnorm, src->nparam,
    ...
    if (this->npoint > 0) {
        ...
        for (i = 0; i < this->npoint; i++) {
            ...
            if (ipoint == NULL) {
                utils::ygglog_error("ObjDict::copy_obj: Could not allocate point %d.", i);
                clear();
                return;
            }
    ...
    if (this->nline > 0) {
        ...
        for (i = 0; i < this->nline; i++) {
            ...
            if (iline == NULL) {
                utils::ygglog_error("ObjDict::copy_obj: Could not allocate line %d.", i);
                clear();
                return;
            }
            this->lines[i] = iline;
        ...
        if (src->line_texcoords == NULL) {
            ...
            for (i = 0; i < this->nline; i++) {
                ...
                if (iline_texcoord == NULL) {
                    ...
                    utils::ygglog_error("ObjDict::copy_obj: Could not allocate line texcoord %d.", i);
                    clear();
                    return;
                }
    ...
    if (this->nface > 0) {
        ...
        for (i = 0; i < this->nface; i++) {
            ...
            if (iface == NULL) {
                utils::ygglog_error("ObjDict::copy_obj: Could not allocate face %d.", i);
                clear();
                return;
            }
            this->faces[i] = iface;
        ...
        if (src->face_texcoords == NULL) {
            ...
            for (i = 0; i < this->nface; i++) {
                ...
                if (iface_texcoord == NULL) {
                    ...
                    utils::ygglog_error("ObjDict::copy_obj: Could not allocate face texcoord %d.", i);
                    clear();
                    return;
                }
    ...
    if (this->ncurve > 0) {
        ...
        for (i = 0; i < this->ncurve; i++) {
            ...
            if (icurve == NULL) {
                utils::ygglog_error("ObjDict::copy_obj: Could not allocate curve %d.", i);
                clear();
                return;
            }
    ...
    if (this->ncurve2 > 0) {
        ...
        for (i = 0; i < this->ncurve2; i++) {
            ...
            if (icurve2 == NULL) {
                utils::ygglog_error("ObjDict::copy_obj: Could not allocate curve2 %d.", i);
                clear();
                return;
            }
    ...
    if (this->nsurf > 0) {
        ...
        for (i = 0; i < this->nsurf; i++) {
            ...
            if (isurface == NULL) {
                utils::ygglog_error("ObjDict::copy_obj: Could not allocate surface %d.", i);
                clear();
                return;
            }
        ...
        if (src->surface_texcoords == NULL) {
            ...
            for (i = 0; i < this->nsurf; i++) {
                int *isurface_texcoord = (int *) realloc(this->surface_texcoords[i],
                                                         src->nvert_in_surface[i] * sizeof(int));
                if (isurface_texcoord == NULL) {

                    utils::ygglog_error("ObjDict::copy_obj: Could not allocate surface texcoord %d.", i);
                    clear();
                    return;
                }
        ...
        if (src->surface_normals == NULL) {
            ...
            for (i = 0; i < this->nsurf; i++) {
                int *isurface_texcoord = (int *) realloc(this->surface_normals[i],
                                                         src->nvert_in_surface[i] * sizeof(int));
                if (isurface_texcoord == NULL) {
...
void ObjDict::display_indent(const char *indent) const {
    int i, j;
    printf("%sMaterial: %s\n", indent, material);
    printf("%s%d Vertices:\n", indent, nvert);
    for (i = 0; i < nvert; i++) {
...
GET_ITEM(obj,ObjDict)

GET_ITEMP(obj,ObjDict)

obj_t init_obj() {
    obj_t x;
    auto p = new communication::datatypes::ObjDict();
...
int alloc_obj(obj_t *p, int nvert, int ntexc, int nnorm, int nparam,
    ...
    if (obj == nullptr)
        return -1;
    return obj->alloc(nvert, ntexc, nnorm, nparam, npoint, nline, nface, ncurve, ncurve2,
                      nsurf, do_color);
}
...
obj_t copy_obj(obj_t src) {
    ...
    if (src.obj == nullptr)
        return out;
    auto osrc = get_obj(src);
    if (osrc == nullptr)
        return out;
    auto obj = new communication::datatypes::ObjDict(osrc);
    out.obj = static_cast<void*>(obj);
    return out;
}
