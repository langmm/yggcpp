...
JSONArrayMetaschemaType::JSONArrayMetaschemaType(const rapidjson::Value &type_doc,
                                                 const char *format_str,
                                                 const bool use_generic,
                                                 const char item_key[100]) :
    ...
    for (i = 0; i < type_doc[item_key_].Size(); i++) {
        MetaschemaType *iitem = (MetaschemaType *) type_from_doc_c(&(type_doc[item_key_][i]),
                                                                   MetaschemaType::use_generic());
        if (iitem == NULL)
...
JSONArrayMetaschemaType::JSONArrayMetaschemaType(PyObject *pyobj, const bool use_generic,
                                                 const char item_key[100]) :
// Always generic
    ...
    for (i = 0; i < nitems; i++) {
        ...
        if (iitem == NULL) {
            ygglog_throw_error(
                    "JSONArrayMetaschemaType: Failed to reconstruct type for item %d from the Python object.", i);
        }
        items.push_back(iitem);
    }
    update_items(items, true);
...
bool JSONArrayMetaschemaType::operator==(const MetaschemaType &Ref) const {
...
void JSONArrayMetaschemaType::display(const char *indent) const {
...
PyObject *JSONArrayMetaschemaType::as_python_dict() const {
    PyObject *out = MetaschemaType::as_python_dict();
    PyObject *pyitems = PyList_New(nitems());
    rapidjson::SizeType i;
    for (i = 0; i < nitems(); i++) {
        PyObject *ipyitem = items_[i]->as_python_dict();
        set_item_python_list(pyitems, i, ipyitem, "JSONArrayMetaschemaType::as_python_dict: items: ",
                             T_OBJECT);
    }
    set_item_python_dict(out, item_key_, pyitems, "JSONArrayMetaschemaType::as_python_dict: ",
                         T_ARRAY);
    return out;
}

Dict *JSONArrayMetaschemaType::copy_generic(const YggGeneric *data, Dict *orig_data) const {
    if (data == nullptr) {
        ygglog_throw_error("JSONArrayMetaschemaType::copy_generic: Generic object is NULL.");
    }
    Dict* new_data = nullptr;
...
void JSONArrayMetaschemaType::free_generic(YggGeneric *data) const {
    if (data == NULL) {
...
void JSONArrayMetaschemaType::display_generic(const YggGeneric *data, const char *indent) const {
    if (data == NULL) {
...
bool JSONArrayMetaschemaType::all_arrays() const {
    bool out = true;
    size_t i;
    if (items_.size() == 0) {
        out = false;
...
void JSONArrayMetaschemaType::update_type_element(size_t i, const MetaschemaType *x) {
    if (i > items_.size()) {
        ygglog_throw_error(
                "JSONArrayMetaschemaType::update_type_element: Cannot add type at index %lu, there are only %lu types present.",
                i, items_.size());
...
void JSONArrayMetaschemaType::update_items(const MetaschemaTypeVector new_items,
                                           bool force) {
    size_t i;
    if (force) {
        free_items();
    ...
    if (items_.size() > 0) {
        if (items_.size() != new_items.size()) {
            ygglog_throw_error(
                    "JSONArrayMetaschemaType::update_items: Cannot update array with %ld elements from an array with %ld elements.",
...
void JSONArrayMetaschemaType::update_use_generic(const bool new_use_generic) {
...
// arrays and objects which must be generic as children).
    ...
    for (i = 0; i < items_.size(); i++) {
        if (items_[i] == NULL) {
            ...
            if ((items_[i]->type_code() == T_ARRAY) || (items_[i]->type_code() == T_OBJECT)) {
...
void JSONArrayMetaschemaType::set_item_type(const size_t index, const MetaschemaType *itemtype) {
    if (index < items_.size()) {
        if ((*(items_[index])) != (*itemtype)) {
            ...
            ygglog_throw_error(
                    "JSONArrayMetaschemaType::set_item_type: New type dosn't match existing type for item %ld",
                    index);
        }
    } else {
...
size_t JSONArrayMetaschemaType::update_from_serialization_args(size_t *nargs, va_list_t &ap) {
    size_t i, iout;
...
// Can't use void* because serialization uses non-pointer arguments
            ...
            if (iva_skip.size() != iout) {
                ygglog_throw_error(
                        "JSONArrayMetaschemaType::update_from_serialization_args: nargs = %lu, size(skip) = %lu",
                        iout, iva_skip.size());
            }
            size_t iskip;
...
size_t JSONArrayMetaschemaType::update_from_deserialization_args(size_t *nargs, va_list_t &ap) {
    size_t i, iout;
    ...
    if ((all_arrays()) && (*nargs >= (nitems() + 1))) {
        ...
        *nrows = items_[0]->nelements();
        for (i = 1; i < items_.size(); i++) {
            ...
            if (*nrows != inrows) {
                ygglog_error(
                        "JSONArrayMetaschemaType::update_from_deserialization_args: Number of rows not consistent across all items.");
                return false;
            }
        }
    }
...
std::vector<size_t> JSONArrayMetaschemaType::nbytes_va_core() const {
    if (!(use_generic())) {
        size_t i;
...
size_t JSONArrayMetaschemaType::nargs_exp() const {
    size_t nargs = 0;
    if (use_generic()) {
        nargs = 1;
    } else {
...
YggGeneric *JSONArrayMetaschemaType::python2c(PyObject *pyobj) const {
    if (!(PyList_Check(pyobj))) {
        ygglog_throw_error("JSONArrayMetaschemaType::python2c: Python object must be a list.");
    }
    if ((size_t) (PyList_Size(pyobj)) != nitems()) {
        ygglog_throw_error(
                "JSONArrayMetaschemaType::python2c: Python list has %lu elements, but the type expects %lu.",
    ...
    for (i = 0; i < nitems(); i++) {
        ...
        if (ipy_item == NULL) {
            ygglog_throw_error("JSONArrayMetaschemaType::python2c: Failed to get item %lu out of the Python list.",
                               i);
        }
...
PyObject *JSONArrayMetaschemaType::c2python(YggGeneric *cobj) const {
    ...
    if (c_items.size() != nitems()) {
        ygglog_throw_error("JSONArrayMetaschemaType::c2python: Type has %lu elements but object has %lu.",
                           nitems(), c_items.size());
    }
    size_t i;
    for (i = 0; i < items_.size(); i++) {
...
bool JSONArrayMetaschemaType::encode_type_prop(rapidjson::Writer<rapidjson::StringBuffer> *writer) const {
    if (!(MetaschemaType::encode_type_prop(writer))) {
        return false;
    }
...
bool JSONArrayMetaschemaType::encode_data(rapidjson::Writer<rapidjson::StringBuffer> *writer,
...
bool JSONArrayMetaschemaType::encode_data(rapidjson::Writer<rapidjson::StringBuffer> *writer,
    ...
    if (arg.size() != items_.size()) {
        ygglog_throw_error("JSONArrayMetaschemaType::encode_data: Type has %d elements, but object has %d.",
    ...
    if (all_arrays()) {
        ...
        for (i = 0; i < items_.size(); i++) {
            if (items_[i]->nelements() != nrows) {
                ygglog_throw_error(
                        "JSONArrayMetaschemaType::encode_data: Element %lu has %lu elements but all array entries are expected to have %lu elements.",
...
bool JSONArrayMetaschemaType::decode_data(rapidjson::Value &data, const int allow_realloc,
                                          size_t *nargs, va_list_t &ap) const {
    size_t i;
    if (!(data.IsArray())) {
        if (items_.size() == 1) {
...
bool JSONArrayMetaschemaType::decode_data(rapidjson::Value &data, YggGeneric *x) {
    size_t i;
    if (all_arrays()) {
        size_t inrows;
        ...
        for (i = 1; i < items_.size(); i++) {
            ...
            if (nrows != inrows) {
                ygglog_error(
                        "JSONArrayMetaschemaType::decode_data: Number of rows not consistent across all items.");
                return false;
            }
        }
    }
