...
class JuliaInterface(Interface):
    ...
    def __init__(self, **kwargs):
        ...
        super(JuliaInterface, self).__init__(
            src_cp[0], language=src_cp[1], added=added, **kwargs)

    # @classmethod
    # def _fix_order(cls, unit):
    #     members_front = []
    #     members = []
    #     for x in unit.properties.get('members', []):
    #         if x.properties.get('name', '').endswith(("Array", "Object")):
    #             members_front.append(x)
    #         else:
    #             members.append(x)
    #     members = members_front + members
    #     if members:
    #         unit.properties['members'] = members

    def final_modified_members(self, solf, out):
        key_methods = ('rapidjson', None, None)
        out.setdefault(key_methods, [])
        if not isinstance(out[key_methods], list):
            out[key_methods] = [out[key_methods]]
        out[key_methods].append(
            RapidjsonDocumentFile._specialize_method_template)
        out[key_methods].append(RapidjsonDocumentFile._wrap_value_returns)
        # Units methods
        key_methods = ('rapidjson', 'units', None, None)
        out.setdefault(key_methods, [])
        if not isinstance(out[key_methods], list):
            out[key_methods] = [out[key_methods]]
        out[key_methods].append(
    ...
    def modify_base(self, base):
        ...
        for cls in (out['rapidjson'].properties['members']
                    + out['rapidjson']['units'].properties['members']):
            if ((cls.properties['name'].endswith(('Array', 'Object',
                                                  'MemberIterator'))
                 and 'Quantity' not in cls.properties['name'])):
                typedefs.append(cls.code_unit('typedef')(
                    name=cls.properties['name'],
                    type=cls.properties['specialized_type']))
            for m in cls.properties.get('members', []):
                ...
                if m.properties['name'] == 'SetString':
                    ...
                    x.properties['type'].properties.update(
                        base=x.code_unit('base_type').parse('std::string'))
                    x.properties['type'].properties['ptr'] = '&'
                    x.properties['type'].properties.pop('template_spec', None)
                    m.properties.pop('wrapped_method', False)
                    m.set_property(
                        'wrapper_body',
                ...
                elif ((m.properties['type'].properties['base'].properties[
                        'name'] == 'char'
                ...
                if ((m.unit_type != 'method'
                     ...
                     or 'wrapped_method' in m.properties)):
                    continue
            members = []
            for m in cls.properties.get('members', []):
                if m.unit_type != 'method':
                ...
                if ((m.properties['name'] in ['String', 'Int', 'Uint',
                                              'Int64', 'Uint64',
                                              'Float', 'Double']
                     or m.properties['type'].properties[
                         'base'].properties['name'] == 'char'
        ...
        if typedefs:
            out['rapidjson'].add_members(typedefs, index=0)
        return out
