...
#     CMAKE_FLAGS_LIB="${CMAKE_FLAGS_LIB} -DCMAKE_PREFIX_PATH=${PREFIX_PATH}"
# fi

if [ -n "$LOCAL_RJ" ]; then
    CMAKE_FLAGS_LIB="${CMAKE_FLAGS_LIB} -DYGGDRASIL_RAPIDJSON_REPO_DIR=${INSTALL_DIR}"
fi
...
if [ -n "$PREFIX_PATH" ]; then
    CMAKE_FLAGS_SPEED="${CMAKE_FLAGS_SPEED} -DCMAKE_PREFIX_PATH=${PREFIX_PATH}"
    CMAKE_FLAGS_DOCS="${CMAKE_FLAGS_DOCS} -DCMAKE_PREFIX_PATH=${PREFIX_PATH}"
fi
...
if [ -n "$DO_DOCS" ]; then
	...
	cd $BUILD_DIR
	cmake .. $CMAKE_FLAGS $CMAKE_FLAGS_DOCS -DYGG_BUILD_DOCS=ON -DBUILD_CXX_LIBRARY=OFF -DBUILD_Fortran_LIBRARY=OFF -DDOXYGEN_CHECK_MISSING=ON
