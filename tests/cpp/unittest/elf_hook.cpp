...
void *elf_hook(char const *module_filename, void const *module_address, const std::string& name, void const *substitution)
{
    ...
    *name_address = NULL;  //address of relocation for symbol named "name"
    ...
    if (NULL == module_address || name.empty() || NULL == substitution) {
        std::cerr << "elf_hook[" << name << "] ERROR in args" << std::endl;
        return original;
    }
    ...
    {  //if something went wrong
        std::cerr << "elf_hook[" << name << "] ERROR in locating symbol" << std::endl;
        free(dynsym);
        free(rel_plt);
        free(rel_dyn);
        free(symbol);
...
//release the data used
    ...
    if (rel_plt) {
      ...
      std::cerr << "elf_hook[" << name << "] rel_plt is NULL" << std::endl;
      rel_plt_amount = 0;
    }
    ...
    if (rel_dyn) {
      ...
      std::cerr << "elf_hook[" << name << "] rel_dyn is NULL" << std::endl;
      rel_dyn_amount = 0;
    }
//release the data used
...
//now we've got ".rel.plt" (needed for PIC) table and ".rel.dyn" (for non-PIC) table and the symbol's index
    ...
    if (original) {
        std::cerr << "elf_hook[" << name << "] Function found in rel.plt orig = " << original << ", sub = " << substitution << std::endl;
        return original;
    }
//we will get here only with 32-bit non-PIC module
    for (i = 0; i < rel_dyn_amount; ++i)  //lookup the ".rel.dyn" table
        ...
        {
	    std::cerr << "elf_hook[" << name << "] function found in rel.dyn" << std::endl;
            ...
            {
	        std::cerr << "elf_hook[" << name << "] ERROR in mprotect 1" << std::endl;
                return NULL;
            }
		...
		std::cerr << "elf_hook[" << name << "] ERROR in mprotect 2" << std::endl;
                return NULL;
            }
        }

    if (!original)
      std::cerr << "elf_hook[" << name << "] ERROR returning NULL" << std::endl;
    return original;
}
