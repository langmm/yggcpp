...
function(extract_target_install_generator_value vraw OUTPUT_VAR)
  set(options)
  set(oneValueArgs COMPILE_LANGUAGE LINK_LANGUAGE PROPERTY_DIR)
  set(multiValueArgs)
  cmake_parse_arguments(ARGS "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
  set(v "${vraw}")
  get_generator_expresion_args("${vraw}" key VALUE_VAR val)
  set(RECURSION_COMPLETE OFF)
  if(key)
    if(key STREQUAL "BUILD_INTERFACE")
      set(v)
    elseif(key STREQUAL "INSTALL_INTERFACE")
      set(v "${val}")
      if(ARGS_PROPERTY_DIR)
        set(vcpy)
        foreach(iv IN LISTS v)
          extract_target_install_generator_value(
            "${iv}" ivcpy
            COMPILE_LANGUAGE ${ARGS_COMPILE_LANGUAGE}
            LINK_LANGUAGE ${ARGS_LINK_LANGUAGE}
          )
          if(ivcpy)
            if(ARGS_PROPERTY_DIR AND NOT IS_ABSOLUTE "${ivcpy}")
              cmake_path(
                APPEND ARGS_PROPERTY_DIR "${ivcpy}"
                OUTPUT_VARIABLE ivcpy
              )
            endif()
            list(APPEND vcpy "${ivcpy}")
          endif()
        endforeach()
        set(RECURSION_COMPLETE ON)
        set(v ${vcpy})
      endif()
    else()
      set(v "${val}")
      ...
      elseif(subkey STREQUAL "LINK_LANGUAGE"
        ...
        set(v)
      elseif(subkey STREQUAL "COMPILE_LANG_AND_ID")
        string(REPLACE "," ";" subval_list "${subval}")
        list(GET subval_list 0 subval_lang)
        if(NOT subval_lang IN_LIST ARGS_COMPILE_LANGUAGE)
          set(v)
        endif()
      elseif(subkey)
  ...
  if(v AND NOT RECURSION_COMPLETE)
    set(vcpy)
    foreach(iv IN LISTS v)
      extract_target_install_generator_value(
        "${iv}" ivcpy
...
function(get_target_install_property OUTPUT_VAR TARGET PROPERTY)
  ...
  set(PROCESSED_OUTPUT)
  foreach(vraw IN LISTS RAW_OUTPUT)
    extract_target_install_generator_value(
      "${vraw}" v
      COMPILE_LANGUAGE ${ARGS_COMPILE_LANGUAGE}
      LINK_LANGUAGE ${ARGS_LINK_LANGUAGE}
      PROPERTY_DIR "${ARGS_PROPERTY_DIR}"
    )
    if(v)
      list(APPEND PROCESSED_OUTPUT ${v})
    endif()
  endforeach()
  foreach(v IN LISTS PROCESSED_OUTPUT)
...
function(complete_install PROJECT)
  ...
  set(multiValueArgs TARGETS POSSIBLE_TARGETS
      HEADERS MODULES_INCLUDE MODULES_EXCLUDE DOCS)
  cmake_parse_arguments(ARGS "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
  if(NOT ARGS_EXPORT)
    set(ARGS_EXPORT ${PROJECT}Targets)
  endif()
  if(NOT ARGS_POSSIBLE_TARGETS)
    set(ARGS_POSSIBLE_TARGETS ${ARGS_TARGETS})
  endif()
  ...
  if(ARGS_TARGETS AND NOT ARGS_DONT_INSTALL_TARGETS)
    if(ARGS_EXPORT_TARGETS_INDEPENDENTLY)
      set(CHILD_EXPORT_PREFIX "${ARGS_EXPORT}Split")
      configure_file(
        ${CMAKE_CURRENT_FUNCTION_LIST_DIR}/config/split_target.cmake.in
        ${ARGS_EXPORT}.cmake @ONLY
      )
      list(
        APPEND ARGS_MODULES_INCLUDE
        "${CMAKE_CURRENT_BINARY_DIR}/${ARGS_EXPORT}.cmake"
      )
      foreach(target IN LISTS ARGS_TARGETS)
        install(
          TARGETS ${target}
          EXPORT ${CHILD_EXPORT_PREFIX}-${target}
        ...
        if(NOT ARGS_DONT_INSTALL_CMAKE_PACKAGING)
          install(
            EXPORT ${CHILD_EXPORT_PREFIX}-${target}
            FILE ${CHILD_EXPORT_PREFIX}-${target}.cmake
            NAMESPACE ${PROJECT}::
