...
macro(_initialize_find_package NO_UNPARSED)
  list(APPEND options REQUIRED GLOBAL)
  ...
  elseif(name MATCHES "Python")
    ...
    if(NOT name STREQUAL "Python3")
      set(name "Python3")
    endif()
    setup_python_search(PREFIX "${name}")
  elseif(name MATCHES "CURL")
    set_default(ARGS_HEADER curl.h)
    set_default(ARGS_LIBNAMES curl libcurl)
    set_default(ARGS_IMPORTED_TARGET CURL::libcurl)
  endif()
...
function(collect_package_arguments VAR PREFIX options)
  collect_arguments(
    ${VAR} ${PREFIX} "${options}"
    REQUIRED GLOBAL FOUND_VAR VAR_PREFIX HEADER IMPORTED_TARGET
...
#         message(DEBUG "Python executable provided ${Python3_EXECUTABLE}")
...
#     endif()
#     if(Python3_ROOT_DIR)
#         message(DEBUG "Python root directory is ${Python3_ROOT_DIR}")
#         if (NOT Python3_ROOT)
...
function(finalize_package name)
  ...
  if(${ARGS_FOUND_VAR} AND name MATCHES "Python")
    ...
    set(Python_EXECUTABLE "${${name}_EXECUTABLE}")
  endif()
  ...
  if(${name}_LIBRARY_TARGET)
    inspect_target(${${name}_LIBRARY_TARGET})
  endif()
...
function(create_interface_library package)
  ...
  add_library(${target} INTERFACE IMPORTED ${ARGS_GLOBAL})
  if(${package}_LIBRARY)
...
function(find_package_default name)
  ...
  if(CMAKE_VERSION VERSION_GREATER_EQUAL "3.24")
    list(APPEND ARGS_UNPARSED_ARGUMENTS ${ARGS_GLOBAL})
  elseif(ARGS_GLOBAL)
    message(WARNING "GLOBAL not supported for find_package in CMake <3.24. \"{name}\" will not be GLOBAL")
  endif()
...
function(find_package_generic name)
  ...
  set(multiValueArgs SEARCH_ORDER COMPONENTS
      HEADER_HINTS HEADER_PATHS BRUTE_SEARCH_ARGS)
  _initialize_find_package(ON ${ARGN})
  ...
  collect_arguments(
    ...
    HEADER_HINTS HEADER_PATHS
  )
  list(APPEND SEARCH_ARGS ${BASE_ARGS})
  list(APPEND SEARCH_ARGS ${ARGS_BRUTE_SEARCH_ARGS})
  collect_arguments(
    DEFAULT_ARGS ARGS "${options}"
    COMPONENTS
  )
  list(APPEND DEFAULT_ARGS ${BASE_ARGS})
  list(APPEND DEFAULT_ARGS ${ARGS_BRUTE_SEARCH_ARGS})
...
function(find_package_brute name)
  set(optionsFind NO_DEFAULT_PATH NO_PACKAGE_ROOT_PATH NO_CMAKE_PATH
...
function(find_package_pkgconfig name)
  ...
  if(ARGS_IMPORTED_TARGET)
    list(APPEND ARGS_PKGCONFIG IMPORTED_TARGET)
    if(CMAKE_VERSION VERSION_GREATER_EQUAL "3.13")
      list(APPEND ARGS_PKGCONFIG ${ARGS_GLOBAL})
    endif()
  endif()
  if(ARGS_LIBNAMES STREQUAL "ZeroMQ")
    message(FATAL_ERROR "ZeroMQ LIBNAMES not provided")
  endif()
  ...
  if(${PN}_FOUND)
    ...
    if(ARGS_IMPORTED_TARGET AND TARGET PkgConfig::${PN})
      add_library(${ARGS_IMPORTED_TARGET} ALIAS PkgConfig::${PN})
    endif()
...
function(find_library_suffix output libname libtype)
  set(options NO_CACHE REQUIRED CREATE_MSVC_IMPORT)
  cmake_parse_arguments(ARGS "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
  if(${output} AND NOT ARGS_NO_CACHE)
    return()
  endif()
  if(NOT (WIN32 AND libtype STREQUAL "IMPORT"))
  ...
  if (ARGS_CREATE_MSVC_IMPORT AND NOT ${output} MATCHES ".*\\.lib$")
    set(${output}_DLL "${output}_DLL-NOTFOUND")
    if(${output} MATCHES ".*\\.dll$")
      set(${output}_DLL "${${output}}")
    elseif(${output} MATCHES ".*\\.dll\\.a$")
      find_library_suffix(
        ${output}_DLL ${libname} SHARED NO_CACHE
        ${ARGS_UNPARSED_ARGUMENTS}
      )
    endif()
    set(${output} "${output}-NOTFOUND")
    if(${output}_DLL MATCHES ".*\\.dll$")
      include(CreateMSVCLib)
      create_msvc_lib_from_dll(${${output}_DLL} OUTPUT ${output})
    endif()
  endif()
  if (${output} STREQUAL "${output}-NOTFOUND")
    if (ARGS_REQUIRED)
...
function(find_libraries)
  set(options CREATE_MSVC_IMPORT)
  ...
  if(NOT (WIN32 AND ARGS_LIBTYPE STREQUAL "IMPORT"))
    set(ARGS_CREATE_MSVC_IMPORT OFF)
  endif()
