...
function(add_yggdrasil_dependency name)
  ...
  if(name MATCHES "Python" AND NOT ARGS_LIBRARIES)
    list(APPEND ARGS_LIBRARIES Python3::NumPy)
    list(APPEND ARGS_LIBRARIES_Python Python3::Python)
  endif()
  ...
  if(${ARGS_FOUND_VAR})
    if(NOT ARGS_LIBRARY_VAR)
      ...
      elseif(${name}_LIBRARIES)
        message(DEBUG "No single library for dependency \"${name}\", adding multiple (${${name}_LIBRARIES})")
        set(ARGS_LIBRARY_VAR ${name}_LIBRARIES)
      endif()
    endif()
    set(IS_TARGET OFF)
    if(ARGS_LIBRARIES)
      if(TARGET ${ARGS_LIBRARIES})
        set(IS_TARGET ON)
      else()
        message(FATAL_ERROR "NO TARGET: ${ARGS_LIBRARY_VAR} = ${${ARGS_LIBRARY_VAR}}")
      endif()
    elseif(ARGS_LIBRARY_VAR)
      if(${ARGS_LIBRARY_VAR} AND TARGET ${${ARGS_LIBRARY_VAR}})
        set(IS_TARGET ON)
      elseif(NOT name STREQUAL "CURL")
        message(FATAL_ERROR "NO TARGET: ${ARGS_LIBRARY_VAR} = ${${ARGS_LIBRARY_VAR}}")
      endif()
    endif()
    if(NOT IS_TARGET)
...
function(find_yggdrasil_dependency name)
  ...
  _propagate_cmake_variables_package()
  if(ARGS_VERBOSE)
    dump_cmake_variables(PREFIX "${ARGS_VAR_PREFIX}" VERBOSE LOG_LEVEL DEBUG)
  endif()
endfunction()
...
function(add_yggdrasil_interface LANGUAGE)
  ...
  set(oneValueArgs DIRECTORY DEPENDENCY LANGUAGE_DEPENDENCY TARGET)
  ...
  if(NOT ARGS_TARGET)
    if(LANGUAGE STREQUAL "CXX")
      if(SKBUILD AND YGG_LINK_Python_TO_CXX)
        set(ARGS_TARGET YggInterface_py)
      else()
        set(ARGS_TARGET YggInterface)
      endif()
    else()
  ...
  set(YGG_TARGET_${LANGUAGE} "${ARGS_TARGET}")
  if(BUILD_${LANGUAGE}_LIBRARY)
    message(STATUS "Building ${LANGUAGE} interface ${ARGS_TARGET}")
  ...
  list(APPEND YGG_INSTALL_TARGETS_SUPPORTED ${ARGS_TARGET})
