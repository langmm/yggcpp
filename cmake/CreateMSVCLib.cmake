...
macro(_initialize_file_transform name src_ext dst_ext)
  list(APPEND options VERBOSE APPEND_TO_OUTPUT)
  ...
  if((NOT SOURCE) AND ARGS_SOURCES)
    ...
    if(${N_SOURCES} GREATER 1 AND NOT ALLOW_MULTIPLE_SOURCES)
      ...
      set(ARGS_APPEND_TO_OUTPUT ON)
      list(FILTER multiValueArgs EXCLUDE SOURCES)
      collect_arguments(
        RECURSION_ARGS ARGS "${options}"
  ...
  if(NOT (TARGET "${SOURCE}" OR EXISTS "${SOURCE}"))
    message(FATAL_ERROR "${name}: SOURCE is not a target or a file: \"${SOURCE}\"")
  endif()
  if(NOT SRC_EXT)
    if(EXISTS "${SOURCE}")
      cmake_path(GET "${SOURCE}" EXTENSION SRC_EXT)
    endif()
  endif()
...
macro(_finalize_file_transform name created)
  ...
  if(ARGS_OUTPUT)
    if(ARGS_APPEND_TO_OUTPUT)
      list(APPEND ${ARGS_OUTPUT} "${ARGS_DESTINATION}")
...
function(elf2coff SOURCE)
  ...
  message(DEBUG "find_program OBJCONV = ${OBJCONV}")
  if(OBJCONV STREQUAL "OBJCONV-NOTFOUND" OR NOT OBJCONV)
    ...
    if(EXISTS ${LOCAL_OBJCONV})
      message(DEBUG "Local objconv exists: ${LOCAL_OBJCONV}")
  ...
  execute_process(
    COMMAND "${OBJCONV}" ${FLAG_VERBOSE} -fcoff ${SOURCE} ${ARGS_DESTINATION}
    COMMAND_ECHO ${ARGS_COMMAND_ECHO}
    RESULT_VARIABLE RET
    OUTPUT_VARIABLE RET_OUTPUT
    ERROR_VARIABLE RET_ERROR
    ECHO_OUTPUT_VARIABLE
    ECHO_ERROR_VARIABLE
    ${ARGS_UNPARSED_ARGUMENTS}
  )
  message(DEBUG "RET_OUTPUT = ${RET_OUTPUT}")
  message(DEBUG "RET_ERROR = ${RET_ERROR}")
  message(DEBUG "RET = ${RET}")
  if(RET)
    message(FATAL_ERROR "RET = ${RET}")
  endif()
  _finalize_file_transform(elf2coef ON)
endfunction()
...
#     message(DEBUG "DDLA2LIB: Could not find ${libname} dll")
#     if (ARGS_OUTPUT)
#       set(${ARGS_OUTPUT} ${dllafile} PARENT_SCOPE)
#     endif()
#     return()
...
#   endif()
#   if(EXISTS "${libfile}")
#     message(DEBUG "DLLA2LIB: ${libfile} already exists")
#     return()
#   endif()
#   if(EXISTS "${deffile}")
#     message(DEBUG "DLLA2LIB: ${deffile} already exists")
#   else()
...
#         message(DEBUG "DLLA2LIB: Stripped def file exists: ${deffile_alt}")
