...
public:
     ...
     */
    explicit Partner(std::string adr) : model(std::move(adr)), signed_off(false) {}
    std::string model;  //!<
    bool signed_off;    //!< Whether we are signed off
};
...
public:
     ...
     */
    Request(std::string  req_id,
            const size_t cidx, bool is_son = false) :
            request_id(std::move(req_id)), data(true), comm_idx(cidx),
...
public:
    ...
    /**
     * @brief Add the given schema to the respons
     * @param[in] s The schema to use
     * @param[in] use_generic If true then initialize to generic values
     */
    ...
    /**
     * @brief Add the given schema to the respons
     * @param[in] metadata The schema to use
     * @param[in] use_generic If true then initialize to generic values
     */
    ...
    /**
     * @brief Add a format to the response
     * @param[in] format_str The format string to use
     * @param use_generic If true then initialize with generic values
     */
     ...
     */
    void transferSchemaTo(Comm_t* comm) const {
