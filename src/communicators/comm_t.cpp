...
comm_t init_comm(const char* name, DIRECTION dir, const COMM_TYPE t,
  ...
  comm_t ret;
  _BEGIN_CPP {
    ret.comm = (void*) YggInterface::communicator::new_Comm_t(dir, t, name, (char*)NULL, COMM_FLAG_INTERFACE);
    if (!(ret.comm))
...
int set_response_format(comm_t comm, const char *fmt) {
  _BEGIN_CPP {
    ...
    static_cast<YggInterface::communicator::RPCComm*>(comm.comm)->addResponseFormat(format_str);
