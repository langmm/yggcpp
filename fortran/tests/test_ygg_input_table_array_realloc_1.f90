integer function test_ygg_input_table_array_realloc_1() result(r)
  use YggInterface
  ...
  if (c_associated(sComm%comm)) then
     if (ygg_send(sComm, [yggarg(nrow_send), yggarg(a_send), &
        ...
        if (ygg_recv_realloc(rComm, [yggarg(nrow_recv), yggarg(a_recv), &
             yggarg(b_recv), yggarg(c_recv)])) then
           if ((nrow_recv.EQ.nrow_send).AND. &
...
end function test_ygg_input_table_array_realloc_1
